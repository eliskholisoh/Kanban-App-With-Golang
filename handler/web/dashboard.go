...
func (d *dashboardWeb) Dashboard(w http.ResponseWriter, r *http.Request) {
	...
	templates,_ := template.New("dashboard.html").Funcs(funcMap).ParseFS(d.embed, dashboard, header)

	if templates.ExecuteTemplate(w, "dashboard.html", dataTemplate) != nil {
		http.Error(w, templates.ExecuteTemplate(w, "dashboard.html", dataTemplate).Error(), http.StatusInternalServerError)
		return
	}
}
